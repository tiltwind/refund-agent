...
| 取判定依据 | 模型绕过或推翻规则引擎的结论 | 越权决策：规则改了也管不住它 |
...
| 检索质量本身（Recall@k 等） | 独立的 [`r1` 检索评测数据集](https://tiltwind.github.io/refund-agent/doc/get-start/4-rag-dataset.md) |
...
| 原始口径 | 遇到的问题 | 收敛结果 |
|---|---|---|
| 引用：只引用检索到的政策条款 | 答复里不写 `P02` 这类文档编号，无法在答复文本上判引用 | 降级为观察指标 `citation_hit`，只判「依据有没有被召回」；逐条引用交给 [`r1`](https://tiltwind.github.io/refund-agent/doc/get-start/4-rag-dataset.md) |
