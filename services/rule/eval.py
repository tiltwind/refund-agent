"""规则服务的 eval 实现 —— 读 evals/data，内含**规则引擎副本**。

线上这套规则跑在独立的规则服务里（1-architecture 第一章）；离线评估连不上它，
于是在这里维护一份等价副本。一条纪律：**规则变更时两边同步**。滞后的副本会让离线
回归拿旧口径给出「与事实相反」的报告 —— 这正是 2-design 6.1 里 ⑤ 那条反向
箭头要防的事：用例挂了，先确认是 Agent 错了还是评估侧的口径滞后了。
"""
