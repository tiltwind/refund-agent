...
| 切片参数 | `320 / 512 / overlap=0`（`rag/chunking/policy.py`） | 参数变了整个 r1 作废，开 r2 |
...
**什么时候开 r2**：切片参数变更、语料大改版、期望值大面积失效。只是补样本就直接往
`cases.jsonl` 里加 —— 版本号碎了就没法做纵向对比。
...
`meta.reviewed` 全部为 `false`：自动自检只能确认「ID 有效、数字没编、分层齐全」，
判不了「真人会不会这么问」。按 10% 抽 10 条，只判三件事，判不了就打回：

1. 这个 query 真人会这么问吗；
2. 不看原文，这个 query 能唯一指向种子块吗 —— 不能说明问得太泛（「退款要多久」这种），
   要改写或降级为 `multi_hop`；
3. 参考答案有没有超出种子块 —— 超出的部分要么删，要么把那个块也加进 `seed_chunk_id`。

抽检过的样本还有第二个用途：Context Recall 靠模型逐条判 claim，判得准不准需要一个基准，
用的就是这批（5-rag-eval 七）。抽检做完之前，两个 LLM 指标的结论不采信。
